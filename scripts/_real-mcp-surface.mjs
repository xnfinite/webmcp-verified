/**
 * _real-mcp-surface.mjs — 14 REAL tools from 5 official MCP servers.
 *
 * The single source of truth for the real-world discovery numbers. Imported by
 * scripts/real-mcp.mjs AND demo/index.html so the measured headline can never
 * diverge between the two (same reason scripts/_surface.mjs is shared).
 *
 * Descriptions are VERBATIM from each server's own source. Property types are
 * source-exact except GitHub's, which come from that server's README table —
 * shapes correct, so say "README-derived" if precision matters. Sources:
 *   modelcontextprotocol/servers            src/{filesystem,fetch,memory,git}
 *   modelcontextprotocol/servers-archived   src/github
 *
 * Why real tools matter here: a real MCP surface has wildly uneven description
 * lengths — filesystem and fetch ship paragraphs, git and memory ship one line.
 * That unevenness is exactly what deferring the long ones pays for, and it is
 * why the real-world saving differs from an illustrative set.
 */
import { defineTool } from "../src/index.js";

/** [server, name, description(verbatim), properties{name:type}, required[]] */
export const realSpecs = [
...
/** Build the real surface as real defineTool tools. Deterministic. */
export function buildRealSurface() {
  return realSpecs.map(([server, name, description, props, required]) =>
    defineTool({
...
/** Server name per tool, in the same order as buildRealSurface(). */
export const realServers = realSpecs.map(([server]) => server);
