 ...
 * plus the break-even N. Prints MEASURED numbers only — asserts nothing by
 * hand. The set is illustrative, not universal: the win scales with (N − used)
 * and schema verbosity, and the composition is inspectable (perTool below).
 *
...
const be = discoveryBreakEven(tools);         // default used=1

console.log(`DISCOVERY COST — ${cost.tools} tools, used=${cost.used}, gauge=${cost.gauge}`);
console.log(
  `${cost.tools} tools cost ${cost.lean.total} tokens to discover the lean way ` +
...
console.log("");
console.log("Naive per-tool:", JSON.stringify(cost.naive.perTool, null, 2));
console.log("");
...
console.log("");
console.log("(estimateTokens is a ~4-char gauge; absolute counts are approximate. savedPct and break-even are the robust figures. Per discovery = per context-load of the tool list, not per tool call.)");
